...
function getCharSize(fontSize,fontBold) {
  ...
  return {width: 16, height: 17.6015625}
}
...
var plotNode={
  ...
  dot:function (node,x,y) {
    ...
    var a=textRect('除换行符以外的任意字符',x,y,bgColor,textColor);
    a.rect.r=10;
  ...
  charset:function (node,x,y) {
    ...
    var clsDesc={d:'数字',D:'非数字',w:'单词',W:'非单词',s:'空白符',S:'非空白符'};
    ...
    if (simple) {
      ...
      if (!node.exclude) {
        ...
        var tl=textLabel(a.x+a.width/2,a.y,'非其中之一:',labelColor);
        var items=a.items;
        items.push(tl.label);
    ...
    var tl=textLabel(rect.x+rect.width/2,rect.y,(node.exclude? '非其中之一:':'其中之一:' ),labelColor);
    items.push(tl.label);
    var oldWidth=width;
    width=Math.max(tl.width,width);
    var offsetX=(width-oldWidth)/2;//ajust label text
